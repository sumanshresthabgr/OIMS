...
<div class="public-view-container">
    ...
    <?php if (!empty($feedbackMessage)): ?>
        <div class="alert <?= $feedbackClass ?>" style="text-align: center; border-radius: 12px; font-size: 0.95rem; margin-bottom: 10px;">
            <?= $feedbackMessage ?>
        </div>
    <?php endif; ?>
        ...
        <div class="card-body">
            <div class="theme-card-layout">
                ...
                <?php if ($rsvp_completed): ?>
                    <div style="border: 1px dashed currentColor; padding: 20px; border-radius: 8px; margin-bottom: 20px; background: rgba(0,0,0,0.02);">
                ...
                <?php if (!empty($event['description'])): ?>
                    <div style="margin-top: 5px;">
                        <p style="font-size: 1.05rem; opacity: 0.95; line-height: 1.6;">
                ...
                <?php if (!empty($event['dress_code']) || !empty($event['registry']) || !empty($event['special_notes'])): ?>
                    <div style="margin-top: 15px; border-top: 1px dashed rgba(0,0,0,0.1); padding-top: 25px; display: grid; gap: 20px; text-align: left;">
                        
                        <?php if (!empty($event['dress_code'])): ?>
                            <div style="display: grid; gap: 4px;">
                                <h5 style="text-align: center; font-size: 0.75rem;">👗 Dress Code</h5>
                                <p style="font-size: 1rem; text-align: center; opacity: 0.85;"><?= htmlspecialchars($event['dress_code']) ?></p>
                            </div>
                        <?php endif; ?>

                        <?php if (!empty($event['registry'])): ?>
                            <div style="display: grid; gap: 4px; text-align: center;">
                                <h5 style="font-size: 0.75rem;">🎁 Gift Registry</h5>
                                <p style="font-size: 1rem;">
                                    <a href="<?= htmlspecialchars($event['registry']) ?>" target="_blank" class="btn btn-theme" style="display: inline-block; padding: 8px 18px; font-size: 0.8rem; text-decoration: none; font-weight:600; text-transform: none;">
                        ...
                        <?php if (!empty($event['special_notes'])): ?>
                            <div style="display: grid; gap: 4px; background: rgba(0,0,0,0.02); padding: 15px; border-radius: 8px;">
                                <h5 style="font-size: 0.75rem; color: currentColor; opacity: 0.7;">⚠️ Important Notes</h5>
                                <p style="font-size: 0.9rem; line-height: 1.5; font-style: italic; opacity: 0.85;">
            ...
            <div class="card-body">
                ...
                <form action="view.php?id=<?= htmlspecialchars($token) ?>" method="POST">
                    <div class="form-group">
                        <label class="form-label">Your Guest Name</label>
                        <input type="text" name="guest_name" class="form-control" placeholder="Enter full name parameter space" required autocomplete="off">
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">Response Track Signature</label>
